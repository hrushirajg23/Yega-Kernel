...
void make_bgdt(bgdesc_t *table) {
    for (uint32_t i = 0; i < g_n_block_groups; i++) {
        ...
        if (i == 0) {
            table[i].bg_free_inodes_count = EXT2_INODES_PER_GROUP - 2;
            table[i].bg_used_dirs_count = 1; // root directory
        } else {
 ...
 *  Block 0    : (unused / boot)
    ...
    Block 8192 : SUPERBLOCK (backup)
    Block 8193 : BGDT (backup)
    Block 8194 : Block bitmap
    Block 8195 : Inode bitmap
    Block 8196–8409 : Inode table
    Block 8410+ : DATA BLOCKS

    Block 16384 : Block bitmap
    Block 16385 : Inode bitmap
    Block 16386–16599 : Inode table
    Block 16600+ : DATA BLOCKS
 */
